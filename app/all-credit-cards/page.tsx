...
import { ArrowRight, Star, TrendingUp, Award, Zap, Search, Filter } from 'lucide-react';
import CardImage from '@/components/CardImage';
import ApplyButton from '@/components/ApplyButton';
import { motion } from 'framer-motion';
import { useState, useMemo, useEffect } from 'react';

export default function AllCreditCardsPage() {
  ...
  const filteredCards = useMemo(() => {
    return creditCards.filter(card => {
      ...
      const matchesFilters = (!selectedFilters.upiRewards || card.upiRewards) &&
                           ...
                           (!selectedFilters.cashback || card.cardType === 'Cashback') &&
                           (!selectedFilters.travel || card.cardType === 'Travel') &&
                           (!selectedFilters.fuel || card.cardType === 'Fuel') &&
                           (!selectedFilters.business || card.cardType === 'Business') &&
                           (!selectedFilters.forex || card.cardType === 'Forex') &&
                           (!selectedFilters.luxury || card.cardType === 'Luxury');
  ...
  const handleSearchChange = (value: string) => {
    ...
    router.push(`/all-credit-cards?${params.toString()}`);
  };
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-lg text-secondary-600 max-w-3xl mx-auto leading-relaxed">
            Discover and compare all credit cards available in India. From cashback to travel rewards, find the perfect card that matches your spending habits and lifestyle.
          </p>
        </motion.div>
        ...
        >
          ...
          {/* Card Type Filters */}
          <div className="mt-4">
            <label className="text-secondary-700 font-medium mb-2 block">Card Types</label>
            <div className="flex flex-wrap gap-2">
              {[
                { key: 'cashback', label: 'Cashback', color: 'blue' },
                { key: 'travel', label: 'Travel', color: 'purple' },
                { key: 'fuel', label: 'Fuel', color: 'orange' },
                { key: 'business', label: 'Business', color: 'green' },
                { key: 'forex', label: 'Forex', color: 'indigo' },
                { key: 'luxury', label: 'Luxury', color: 'pink' },
              ].map(({ key, label, color }) => (
                <button
                  key={key}
                  onClick={() => handleFilterToggle(key as keyof typeof selectedFilters)}
                  className={`inline-flex items-center gap-1 px-3 py-2 rounded-lg text-sm font-medium transition-all ${
